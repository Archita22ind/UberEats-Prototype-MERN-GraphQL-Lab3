...
const Orders = (props) => {
  ...
  const getReceiptDetails = async (orderId) => {
      ...
      const response = await fetch(
          "http://10.0.0.8:8080/getReceiptDetails",
          {
            method: "POST",
            headers: {
  ...
  let orderFilterOptions = ["", "Ordered", "Order Received", "Preparing",  "Delivered"];
...
const formatTextForOrders = (date, quantity, price) => {
    ...
    return parseDate ?  (<p>{`${quantity} items for $${price} on ${parseDate.toLocaleString('en-US', { timeZone: 'PST' })}`}</p>) 
...
const displayOrders = () => {
  return ordersList?.length>0 ? 
   (ordersList.map ((pastOrder) => {
    return <Row> <ListGroup.Item>
      <Col><h5>{pastOrder.restaurantName}</h5></Col>
      <Col>{formatTextForOrders(pastOrder.dateOrdered, pastOrder.totalItems, pastOrder.totalPrice)}</Col>
      <Col> <Button
          variant="light"
  ...
  ) : (<ListGroup.Item><h5>No Orders Found !!</h5></ListGroup.Item>)
}

const getPastOrders = async () => {

  const response = await fetch(
    "http://10.0.0.8:8080/getPastOrders",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
